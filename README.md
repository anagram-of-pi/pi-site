...
After I finished the source code (which is about 10KB, give or take) I started minifying. I copied it from `src/` into `minified_src/` to save my progress and started out by reducing it by hand. 
...
Finally, as the last step, I ran all three files (`index.html`, `style.css` and `script.js`) through an automatic minimizer to remove whitespace. 
...
No AI was used in making the site itself or any of the content. AI was briefly used to target which CSS selectors could be factored out of properties in order to minify it. 
This README was written 100% by myself, @Pi, without any use of AI other than spell check. 